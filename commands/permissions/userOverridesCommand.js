...
module.exports = class userOverridesCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      name: "user-override",
      group: "permissions",
      memberName: "user-override",
      description: "",
      examples: [],
      guildOnly: true,
      argsType: "multiple",
  ...
  async run(receivedMessage, args) {
    ...
    if (args.length < 3 && args[0].toLowerCase() !== 'clear' ) {
        ...
        if (receivedMessage.mentions.members.first()) {
            user = receivedMessage.mentions.members.first()
        }
        else {
            user = await receivedMessage.guild.members.fetch(args[0])
        }
        args.splice(0,1)
        ...
        if (!allowedOptions.includes(option)){
            ...
            if (!user) {
                return receivedMessage.say('no user')
            }
            else {
                const userOverrides = receivedMessage.guild.guildSettings.permissionsSettings.roleOverrides
