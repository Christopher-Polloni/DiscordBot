...
module.exports = class enableCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      ...
      examples: ['enable <command>', 'enable <command category>'],
      guildOnly: true,
      argsType: "single",
    ...
    if (arg) {
      if (this.client.registry.findCommands(arg).length > 0) {
        ...
        if (receivedMessage.guild.guildSettings.permissionsSettings.enabledCommands.includes(command.name)) {
            embed.setDescription(`<:check_mark:845062687649562625> The command \`${command.name}\` is already enabled throughout the server.`)
            ...
            embed.setDescription(`<:check_mark:845062687649562625> The command \`${command.name}\` is now enabled throughout the server.`)
        ...
        if (receivedMessage.guild.guildSettings.permissionsSettings.disabledCommandGroups.includes(group.name)) {
            ...
            embed.setDescription(`<:check_mark:845062687649562625> The \`${group.name}\` category is now enabled throughout the server.`)
          ...
          embed.setDescription(`<:check_mark:845062687649562625> The \`${group.name}\` category is already enabled throughout the server.`)
          return receivedMessage.say(embed)
        }
      }
      else {
